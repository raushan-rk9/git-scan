class LowLevelRequirement < OrganizationRecord
  ...
  has_and_belongs_to_many :high_level_requirements, -> { distinct }, join_table: "hlr_llrs"

  # Associate TCs with LLRs
  has_and_belongs_to_many :test_cases,              -> { distinct }, join_table: "llr_tcs"

  # Associate MDs with LLRs
  has_and_belongs_to_many :module_descriptions,     -> { distinct }, join_table: "llr_mds"
  ...
  # Generate Item full_id + reqid + item_id
  def long_id
...
public
  def self.rename_prefix(project_id, item_id, old_prefix, new_prefix,
    ...
    project.low_level_requirements_prefix = new_prefix
    item.low_level_requirements_prefix    = new_prefix
    ...
    low_level_requirements                = LowLevelRequirement.where(project_id:     project_id,
                                                                        item_id:      item_id,
                                                                        organization: User.current.organization)

    low_level_requirements.each do |low_level_requirement|
      if low_level_requirement.full_id.present?
        next unless low_level_requirement.full_id.sub!(old_prefix,
                                                       new_prefix).present?
      else
        low_level_requirement.full_id     = low_level_requirement.fullreqid
      end

      return false unless DataChange.save_or_destroy_with_undo_session(low_level_requirement,
                                                                       'update',
  ...
  def self.renumber(item_id,
    ...
    prefix                           = prefix.sub(/\-$/, '')
    session_id                      = nil
    low_level_requirements          = LowLevelRequirement.where(item_id:      item_id,
                                                                organization: User.current.organization).order(:reqid)
    maximum_reqid                   = LowLevelRequirement.where(item_id:      item_id,
                                                                 organization: User.current.organization).maximum(:reqid)
    maximum_reqid                  += 1

    low_level_requirements.each do |low_level_requirement|
      low_level_requirement.reqid   = start
      low_level_requirement.full_id = prefix + sprintf("-%0*d", padding, start)
      existing_record               = LowLevelRequirement.find_by(reqid:        start,
                                                                   item_id:      item_id,
                                                                   organization: User.current.organization)
      ...
      change_record                 = DataChange.save_or_destroy_with_undo_session(low_level_requirement,
                                                                                   'update',
                                                                                   low_level_requirement.id,
                                                                                   'low_level_requirements', 
                                                                                   session_id)
  ...
  # CSV file handling
  DEFAULT_HEADERS = %w{id reqid full_id description module_description derived version item_id project_id high_level_requirement_associations derived_justification created_at updated_at organization category verification_method safety soft_delete document_id model_file_id archive_revision archive_version }
  ...
  def self.to_csv(item_id, headers = DEFAULT_HEADERS)
    ...
    CSV.generate(headers: true) do |csv|
      ...
      RequirementsTracing.sort_on_full_id(low_level_requirements)

      low_level_requirements.each { |llr| csv << llr.get_columns(true, headers) }
    end
  end
  ...
  def self.to_xls(item_id, headers = DEFAULT_HEADERS)
    ...
    low_level_requirements.each do |llr|
      xls_worksheet.insert_row(current_row, llr.get_columns(false, headers))

      current_row += 1
    end

    file = Tempfile.new('low_level-requirements')
  ...
  # The normal Usage would be: LowLevelRequirement.from_csv('filename.csv'
  #                                                          @item)
  ...
  # Description: Assigns a value to a specic value in the LowLevelRequirement
  # Calls:       None
  # Notes:       
  # History:     06-10-2019 - First Written, PC
  ...
  #              :duplicate_low_level_requirement if the id is a duplicate;
  ...
  def self.process_row(columns,
    ...
    low_level_requirement              = self.find_or_create_low_level_requirement_by_id(columns,
                                                                                         item,
                                                                                         headers)

    return result unless low_level_requirement.present?

    if check_download.include?(:check_duplicates) && low_level_requirement.id.present?
      return :duplicate_low_level_requirement
    end

    high_level_requirement_associations = low_level_requirement.high_level_requirement_associations
    ...
    columns.each_with_index do |column, index|
      ...
      result                            = low_level_requirement.assign_column(column_name,
                                                                               column,
    ...
    if check_download.include?(:check_associations)                 &&
       (high_level_requirement_associations.present?                &&
        (low_level_requirement.high_level_requirement_associations !=
         high_level_requirement_associations))
      return :high_level_requirement_associations_changed
    end

    if check_download.empty?
      low_level_requirement.full_id    = "LLR-#{low_level_requirement.reqid}" if !low_level_requirement.full_id.present? && low_level_requirement.reqid.present?
      operation                         = low_level_requirement.id.present? ? 'update' : 'create'
      change_record                     = DataChange.save_or_destroy_with_undo_session(low_level_requirement,
                                                                                       operation,
                                                                                       low_level_requirement.id,
                                                                                       'low_level_requirements',
      ...
      if change_record.present?
        result                          = Associations.build_associations(low_level_requirement)
...
private
  ...
  def self.find_or_create_low_level_requirement_by_id(columns,
                                                       item    = @item,
                                                       headers = DEFAULT_HEADERS)
    ...
    if id.present?
      low_level_requirement = LowLevelRequirement.find_by(reqid:   id,
                                                          item_id: item.id)
    else
      last_id             = nil
      low_level_requirements = LowLevelRequirement.where(item_id:  item.id, organization: User.current.organization).order("reqid")
    ...
    unless low_level_requirement.present?
      item.llr_count                   += 1
      ...
      low_level_requirement.reqid      = id
    end

    return low_level_requirement
  end
end
