...
{
  CMARK_NANOSECONDS(16);
}

void function_call_2()
{
  CMARK_NANOSECONDS(17);

  return;

  CMARK_NANOSECONDS(18);
}

void function_call_3()
{
  CMARK_NANOSECONDS(19);

  exit(0);

  CMARK_NANOSECONDS(20);
}
...
int main (int argc, char *argv[])
{
  ...
  if (test_while)
  {
    ...
    CMARK_NANOSECONDS(21);

    while (!done)
    {
      CMARK_NANOSECONDS(22);

      done = TRUE;
    }

    CMARK_NANOSECONDS(23);
  }

  if (test_for)
  {
    CMARK_NANOSECONDS(24);

    for (int i = 0; i < 1; i++)
      CMARK_NANOSECONDS(25);

    CMARK_NANOSECONDS(26);
  }
  ...
  if (test_do_while)
  {
    ...
    CMARK_NANOSECONDS(27);

    do
    {
      CMARK_NANOSECONDS(28);

      done = TRUE;
    } while (!done);

    CMARK_NANOSECONDS(29);
  }
  ...
  if (test_switch)
  {
    ...
    CMARK_NANOSECONDS(30);

    switch (i)
    {
      case 1:
        CMARK_NANOSECONDS(31);

        break;

        if (test_break)
          CMARK_NANOSECONDS(32);

      default:
        if (test_default)
          CMARK_NANOSECONDS(33);
    }

    CMARK_NANOSECONDS(34);
  }

  CMARK_NANOSECONDS(99);
