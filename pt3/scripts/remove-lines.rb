...
def parse_options
  optparse                       = OptionParser.new do|opts|
    ...
    # of the help screen.
    opts.banner                  = "Usage: search-remove.rb [options]"
    ...
    @options[:display_removals]  = false
    @options[:remove_count]      = 1
    ...
    opts.on('-V', '--verbose', '--display-removals') do
      @options[:display_removals] = true
    end
    ...
    opts.on('-p', '--putback', 'Replace Original File after processing') do
      @options[:replace_original] = true
    end

    opts.on("-s", "--search=SEARCH", "String to search for") do |search|
      @options[:search]      = search
    end

    opts.on("-x", "--exclude=EXCLUDE", "Exclude replacement if string exists") do |exclude|
      @options[:exclude]     = exclude
    end

    @options[:input]         = nil

    opts.on("-i", "--input-file=INPUT", "Input file to process") do |input|
      @options[:input]       = input
    end

    @options[:output]        = nil

    opts.on('-o', '--output-file=OUTPUT', 'Output file to process') do |output|
      @options[:output]      = output
    end

    opts.on("-c", "--remove-count=COUNT", "Number of lines to remove") do |count|
      @options[:remove_count] = count.to_i if count =~ /^\d+$/
    end
...
def process_file
  ...
  @remove_count     = if @options[:remove_count].nil?
                        1
                      else
  ...
  File.open(@options[:input], 'r')    do |input_file|
    if @options[:output].nil? || (@options[:output].downcase == 'stdout')
      while (line   = input_file.gets)
        if !(!@exclude.nil? && line.index(@exclude)) && line.index(@search)
          puts "Removing: #{line}" if @options[:display_removals]

          if @remove_count > 1
            for index in 2..@remove_count
              line = input_file.gets if @options[:display_removals]
...
def search_remove
  ...
  elsif @options[:display_removals]
    puts("Searching #{@options[:input]}")
  end
